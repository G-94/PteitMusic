...
DownloadsService::DownloadsService(MusicApi* api_) : api{api_}
{
...
void DownloadsService::addSongToDownloads(const Song &song)
{
...
void DownloadsService::removeSongFromDownloads(std::string songID)
{
    std::string artist;
    std::string title;
...
void DownloadsService::loadDownloadsSongs()
{
...
void DownloadsService::saveDownloadsSongsToJson()
{
    try {
        json j = json::array();
...
std::string DownloadsService::clearPathName(const std::string &path)
{
    std::string r = path;
    for(char& c : r) {
        if(c == '/' || c == '\\' || c == ':' || c == '*' ||
